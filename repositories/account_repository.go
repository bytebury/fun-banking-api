...
func (repository AccountRepository) GetTransferHistoricalData(accountID string, daysAgo int) ([]models.DailyTransferSummary, error) {
	var dailySummaries []models.DailyTransferSummary
	xDaysAgo := time.Now().AddDate(0, 0, -daysAgo)

	subQuery := repository.db.Model(&models.Transfer{}).
		Select("MAX(updated_at) as max_updated_at").
		Where("updated_at >= ? AND account_id = ?", xDaysAgo, accountID).
		Group("DATE(updated_at)")

	result := repository.db.Model(&models.Transfer{}).
